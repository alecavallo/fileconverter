...
const convert = {
   ...
   * @returns {smallThumbBuffer: Promise, bigThumbBuffer: Promise} Object with 2 promises one for the smal thumbnails and the other for the big thumbnails. Both are resolved into a buffer
   */
                ...
                resolve(
                  Promise.all(fileUploadPromise)
                    ...
                    .catch(err => {
                      console.log(err);
                      console.log(err.stack);
                      //disconnecting from firebase after processing
                      firebase.disconnect();
                    })
                );
              }
  ...
  //private
  /**
   * Method called to upload a file to an S3 bucket
   ...
   * @returns {Promise} Buffer with the resized image
   */
   ...
   * @param {Buffer|String} input A buffer or the path to a file to be compressed
   * @param {util.constants} fileType Mimetype of the file
   * @returns {Promise} Buffer with the compressed file
   */
